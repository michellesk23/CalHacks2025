// Pipe normalized JSON into the Python eligibility checker and print the result.
// Usage:
//   FDC_API_KEY=... node check_with_python.js <barcode>

const { spawn } = require('child_process');
const isWin = process.platform === 'win32';
const PYTHON_BIN = process.env.PYTHON || (isWin ? 'python' : 'python3');
...
function toPythonInput(p) {
  ...
  return {
    ...
    nutrients: {
      total_sugars_g: (p.nutriments && (p.nutriments.sugars_100g ?? p.nutriments.sugars ?? p.nutriments.sugars_serving)) || null
    },
...
async function main() {
  ...
  try {
    ...
    const py = spawn(PYTHON_BIN, ['eligibility/run_check.py'], { stdio: ['pipe', 'inherit', 'inherit'] });
